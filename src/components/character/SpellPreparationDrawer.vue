...
const toggleSpellPrepared = (spell: any) => {
  if (spell.is_always_prepared) return;
    ...
    <div class="relative w-full max-w-5xl h-full max-h-[85vh] bg-zinc-900 border border-zinc-700 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
      ...
      <div class="p-6 border-b border-zinc-800 flex flex-wrap items-center justify-between bg-zinc-900/50 gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-xl font-black text-white">法术整备</h3>
            <p class="text-xs text-zinc-500 font-bold uppercase">常规限额: <span :class="preparedCount > prepLimit ? 'text-red-500' : 'text-blue-400'">{{ preparedCount }}</span> / {{ prepLimit }}</p>
          </div>
        </div>
        <div class="flex items-center gap-4">
          <div class="relative w-64">
            <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
            <input v-model="searchQuery" placeholder="搜索法术..." class="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-10 pr-4 py-2 text-xs text-zinc-200 outline-none" />
          </div>
          <button @click="emit('close')" class="p-2 hover:bg-zinc-800 rounded-full"><X class="w-6 h-6 text-zinc-500" /></button>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          >
            <button 
              ...
              class="w-8 h-8 rounded-lg flex items-center justify-center border shrink-0 transition-all disabled:opacity-50"
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-[8px] text-zinc-500 font-bold uppercase tracking-tighter mt-0.5">{{ spell.level === 0 ? '戏法' : spell.level + ' 环' }}</div>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <button 
                ...
                :class="spell.is_always_prepared ? 'bg-amber-500/20 border-amber-500/50 text-amber-400' : 'bg-zinc-900 border-zinc-700 text-zinc-600 hover:text-amber-500'"
              >
